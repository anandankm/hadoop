...
import com.grooveshark.hadoop.util.FileUtils;
import com.grooveshark.hadoop.util.DateUtils;
...
public class MysqlLoad
{
    ...
    public class MysqlLoadWorker extends Configured implements Runnable, Tool
    {
        ...
        public String mysqlJar = "/user/hdfs/oozie/libs/mysql-connector-java-5.1.18-bin.jar";
        public String name;
        public boolean updatePropertyBag = false;
        public String[] args;
        ...
        public int run(String[] args)
            throws IOException
        {
            logToStdOut(this.name, "Starting the job.");
        ...
        while (it.hasNext()) {
            ...
            if (!(t.isAlive())) {
                ...
                if (this.updatePropertyBag) {
                    try {
                        MysqlLoad.updatePropertyBagTable(tableInfoArgs[0], this.mysqlAccess, tableSuffix);
                    } catch (SQLException e) {
    ...
    public static int updatePropertyBagTable(String tableID, MysqlAccess mysqlAccess, String tableSuffix)
        throws SQLException
    {
        String sql = "UPDATE BelugaPropertyBag set Value = '" + tableSuffix + "' WHERE Property = '" + tableID + "'";
        return mysqlAccess.executeUpdate(sql);
    }
